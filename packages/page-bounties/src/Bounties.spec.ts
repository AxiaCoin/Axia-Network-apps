...
import type { Bounty, BountyIndex, BountyStatus } from '@axia-js/types/interfaces';

import { fireEvent } from '@testing-library/react';
import BN from 'bn.js';

import { ApiPromise } from '@axia-js/api';
import i18next from '@axia-js/react-components/i18n';
import { createAugmentedApi } from '@axia-js/test-support/api';
...
import { proposalFactory } from '@axia-js/test-support/creation/treasury/proposalFactory';
import { mockHooks } from '@axia-js/test-support/hooks/mockHooks';
...
jest.mock('@axia-js/react-hooks/useTreasury', () => ({
  useTreasury: () => mockHooks.treasury
}));

jest.mock('@axia-js/react-hooks/useCollectiveInstance', () => ({
  useCollectiveInstance: () => 'council'
}));

jest.mock('@axia-js/react-hooks/useCollectiveMembers', () => ({
  useCollectiveMembers: () => mockHooks.members
}));

jest.mock('@axia-js/react-hooks/useBlockTime', () => ({
...
describe('Bounties', () => {
  ...
  describe('list', () => {
    ...
    it('renders a bounty', async () => {
      bountiesPage.renderOne(aBounty(), [], 'axialunar comic book');

      await bountiesPage.expectText('axialunar comic book');
